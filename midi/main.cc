...
int main(int, char**)
{
	...
	std::fstream file;
	file.open("mk.mid", std::ios::in | std::ios::binary);
	...
	while (ptr < track[trk].size()){
		...
		switch(event_type) {
			...
			case 0x90 ... 0x9f:{
				...
				fg->set_freq(s_freq_table[note]);
				fg->set_volume(velocity/127.);
				std::cout << "Включить ноту " << note << "с громкостью " << velocity << std::endl;
				std::cout.flush();
				fg->key_on();
			}break;
			...
			case 0xff:{
				...
				switch (meta_type) {
					...
					case 0x01:{
						...
						std::cout << "Эта дорожка называется: " << text << std::endl;
					}break;
